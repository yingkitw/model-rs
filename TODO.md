...
- [ ] Confirm `cargo test` and `cargo build --release` on a clean machine after freeing disk space (CI optional). E2E tests resolve the CLI via runtime `CARGO_BIN_EXE_model_rs` so they stay valid if the repo path changes.
