...
- [x] All root docs (README, SPEC, ARCHITECTURE, AGENTS, TODO) updated to reflect current implementation: built-in GGUF, removed MLX/CUDA, single `metal` feature, 227 passing tests.
...
- [x] **Removed vendored candle-core** — switched back to upstream crates.io `candle-core` 0.10.2. The `[patch.crates-io]` override and `vendor/` directory were removed. This accepts the transitive `onig` (C regex) dependency from `candle-core`'s `tokenizers`, requiring a C compiler to build.
  ...
  - Removed `llama_cpp` (C++ llama.cpp bindings): rewrote GGUF backend to use candle-core's `quantized::gguf_file` + candle-transformers' `quantized_llama::ModelWeights`.
  - Removed `mlx-rs` (C++ MLX framework): MLX backend replaced with error stub; Metal backend provides GPU acceleration on Apple Silicon.
  ...
  - Only remaining feature: `metal` (default) — Metal FFI via `metal` crate.
  - Note: `candle-core`'s transitive `tokenizers` dependency still uses `onig` (C regex), requiring a C compiler. The previous `vendor/candle-core` patch to switch to `fancy-regex` was removed in favor of using upstream crates.
