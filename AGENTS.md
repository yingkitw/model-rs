...
This document defines the continuous improvement cycle for the **model-rs** LLM inference toolkit.
...
- Write integration tests (e.g. `tests/integration_test.rs`, `tests/e2e_test.rs`, `tests/api_error_test.rs`, `tests/error_handling_test.rs`) that exercise the feature end-to-end
...
Research similar open-source LLM inference tools and serving frameworks (Ollama, vLLM, SGLang, llama.cpp, Candle-based projects, etc.). Identify capabilities they have that this project lacks. Add the most valuable ones to the `TODO.md` brainstorming section. Prioritize features that provide clear competitive advantage.

### 7. Update Documentation
Keep all project docs aligned with the current implementation. Root docs (required):

- **`README.md`**: Quick start, CLI options (`model-rs` binary), feature list, architecture summary
- **`TODO.md`**: Mark completed items, move them to Done, keep brainstorming current
- **`SPEC.md`**: Functional requirements, CLI semantics, HTTP API, error handling
